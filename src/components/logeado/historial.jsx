...
const ConsultasEst = () => {
    ...
    const { quotes, fetchUserQuotesToday } = useQuoteStore();
    const token = localStorage.getItem('token');
    const userId = localStorage.getItem('userId');

    useEffect(() => {
        if (userId && token) {
            fetchUserQuotesToday(userId, token);
        }
    }, [userId, token, fetchUserQuotesToday]);
            ...
            <div className="flex flex-col lg:flex-row justify-center items-center gap-10">
                <div className="container mx-auto mt-32">
                    ...
                    >
                        ...
                        <p className="text-tartiary md:w-1/3 mx-auto px-4">Registro de todas las consultas terapéuticas que realizo el estudiante.</p>
                    </motion.div>
...
const AppointmentCard = ({ appointment, onOpenDetailModal }) => {
    ...
    return (
        <div className="bg-white shadow rounded-lg p-4 flex flex-col mb-4 relative">
            <img src={therapist1} alt="Icon Name" className="w-10 h-10 self-start" />
