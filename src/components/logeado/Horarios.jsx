...
import therapist3 from '../../assets/profile3.png';
...
const Información = () => {
    ...
    return (
        <div className="my-0 md:px-14 px-4 max-w-screen-2xl mx-auto" id='servicios'>
            ...
            <HorarioModal isOpen={modalOpen} onClose={() => setModalOpen(false)} />
...
const TherapistCard = ({ therapist, onOpenModal, onOpenDetailModal }) => {
    ...
    return (
        <div className="relative w-full h-90 flex flex-col justify-center items-center p-6 bg-white rounded-[35px] shadow-3xl">
            <img src={therapist.image} alt={therapist.name} className="w-2/6 mb-4" />
            <h5 className="text-xl md:text-2xl lg:text-2xl text-primary font-semibold mt-4 text-center">{therapist.name}</h5>
            <p className="text-center mt-2">{therapist.info}</p>
            ...
            <button onClick={() => onOpenModal(true)} className="mt-4 px-4 py-2 text-white bg-secondary rounded hover:bg-primary-dark transition-colors duration-300">
                Solicita una reserva
            </button>
            <button onClick={() => onOpenDetailModal(therapist)} className="mt-2 px-4 py-2 text-primary bg-gray-200 rounded hover:bg-secondary-dark transition-colors duration-300">
                Más información
            </button>
        </div>
    );
